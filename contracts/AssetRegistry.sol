...
contract AssetRegistry {
  ...
  event AssetCreated(uint assetId,uint block,uint borough,uint lot,	address currentOwner);
  event UpdateAsset(address currentOwner,uint assetId);
  ...
  function createAsset(uint block, uint borough, uint lot, address currentOwner) onlyowner {
	...
	AssetCreated(counter,block,borough,lot,currentOwner);
  }
  ...
  function updateAssetInfo(address newOwner, uint assetId) onlyowner {
      ...
      UpdateAsset(asset.currentOwner, assetId);
  }
